...
pub fn vis(input: &Input, out: &Output, t: i32, show_pair: bool, show_all: bool) -> String {
    ...
    for i in 0..out.path.len() {
        if i > 0 {
            ...
            let data = if time + d > t {
                ...
                Data::new().move_to(out.path[i - 1]).line_by((
                    (out.path[i].0 - out.path[i - 1].0) as f64 * mul,
                    (out.path[i].1 - out.path[i - 1].1) as f64 * mul,
                ))
            } else {
    ...
    for i in 0..N {
        if status[i] != !0 {
            let c = if status[i] >= 1 { "red" } else { "red" };
            let circle = Circle::new()
                .set("cx", input.from[i].0)
                .set("cy", input.from[i].1)
                .set("r", 6)
                .set("fill", c);
            doc = doc.add(circle);
            let c = if status[i] == 2 {
