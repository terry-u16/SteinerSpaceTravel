...
const char *latestFeatures[] = {
    ...
    "checker.testset() to get values",
    "Added quitpi(points_info, message) function to return with _points exit code 7 and given points_info",
    "rnd.partition(size, sum[, min_part=1]) returns random (unsorted) partition which is a representation of the given "
    "`sum` as a sum of `size` positive integers (or >=min_part if specified)",
    ...
    "Reformatted",
    "Use setTestCase(i) or unsetTestCase() to support test cases (you can use it in any type of program: generator, "
    ...
    "Added expectedButFound quit function, examples: expectedButFound(_wa, 10, 20), expectedButFound(_fail, ja, pa, "
    "\"[n=%d,m=%d]\", n, m)",
    "Fixed incorrect interval parsing in patterns",
    "Use registerGen(argc, argv, 1) to develop new generator, use registerGen(argc, argv, 0) to compile old generators "
    "(originally created for testlib under 0.8.7)",
    ...
    "Use -DENABLE_UNEXPECTED_EOF to enable special exit code (by default, 8) in case of unexpected eof. It is good "
    "idea to use it in interactors",
...
#endif
#else
        if (file == stdin) freopen(NULL, "rb", file);
        if (file == stdout || file == stderr) freopen(NULL, "wb", file);
#endif
    }
}
...
class random_t {
   ...
   public:
    ...
    std::vector<T> distinct(int size, T from, T to) {
        ...
        if (uint64_t(size) > n) __testlib_fail("random_t::distinct expected size <= to - from + 1");
        ...
        if (expected < double(n)) {
            ...
            if (n > 1000000000) __testlib_fail("random_t::distinct here expected to - from + 1 <= 1000000000");
    ...
    std::vector<T> distinct(int size, T upper) {
        ...
        if (size > upper) __testlib_fail("random_t::distinct expected size <= upper");
    ...
    std::vector<T> partition(int size, T sum, T min_part) {
        ...
        if (size == 0 && sum != 0) __testlib_fail("random_t::partition: size == 0 && sum != 0");
        if (min_part * size > sum) __testlib_fail("random_t::partition: min_part * size > sum");
        ...
        for (int i = 0; i + 1 < size; i++) result[i] = septums[i + 1] - septums[i] - 1;
        ...
        if (result_sum != sum_) __testlib_fail("random_t::partition: partition sum is expected to be the given sum");
...
class Validator {
   ...
   public:
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail(
                "Validator should be initialized with registerValidation(argc, argv) instead of registerValidation() "
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (resultName != "") {
        ...
        if (appesMode) {
            ...
            if (isPartial)
                std::fprintf(resultFile, "<result outcome = \"%s\" pctype = \"%d\">", outcomes[(int)_partially].c_str(),
                             pctype);
            else {
                if (result != _points)
                    ...
                    std::fprintf(resultFile, "<result outcome = \"%s\" points = \"%s\">", outcomes[(int)result].c_str(),
                                 stringPoints.c_str());
                }
            }
...
static inline double stringToStrictDouble(InStream &in, const std::string &buffer, int minAfterPointDigitCount,
                                          int maxAfterPointDigitCount) {
    for (size_t i = 0; i < buffer.length(); i++)
        if (buffer[i] == '\0')
            in.quit(_pe, ("Expected double, but \"" + __testlib_part(buffer) + "\" found (it contains \\0)").c_str());
...
static inline long long stringToUnsignedLongLong(InStream &in, const std::string &buffer) {
    for (size_t i = 0; i < buffer.length(); i++)
        if (buffer[i] == '\0')
            in.quit(
                _pe,
                ("Expected unsigned integer, but \"" + __testlib_part(buffer) + "\" found (it contains \\0)").c_str());
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                ...
                quit(_wa,
                     ("Integer parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) +
                      ", violates the range [" + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                         .c_str());
        } else {
            if (variableName.empty())
                quit(_wa,
                     ("Integer element [index=" + vtos(readManyIteration) + "] equals to " + vtos(result) +
                      ", violates the range [" + toHumanReadableString(minv) + ", " + toHumanReadableString(maxv) + "]")
                         .c_str());
            else
...
unsigned long long InStream::readUnsignedLong(unsigned long long minv, unsigned long long maxv,
    ...
    if (result < minv || result > maxv) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                quit(_wa, ("Unsigned integer " + vtos(result) + " violates the range [" + toHumanReadableString(minv) +
                           ", " + toHumanReadableString(maxv) + "]")
                              .c_str());
            else
                quit(_wa,
...
NORETURN void quitpi(const std::string &points_info, const std::string &message = "") {
    if (points_info.find(' ') != std::string::npos) quit(_fail, "Parameter 'points_info' can't contain spaces");
    if (message.empty())
...
class Checker {
   ...
   public:
    ...
    std::string testset() const {
        if (!_initialized)
            __testlib_fail(
                "Checker should be initialized with registerTestlibCmd(argc, argv) instead of registerTestlibCmd() to "
                "support checker.testset()");
        return _testset;
    }
...
void registerTestlibCmd(int argc, char *argv[]) {
    ...
    if (argc < 4 || argc > 6) {
        quit(_fail, std::string("Program must be run with the following arguments: ") +
                        std::string("[--testset testset] [--group group] <input-file> <output-file> <answer-file> "
                                    "[<report-file> [<-appes>]]") +
                        "\nUse \"--help\" to get help information");
    }
...
template <>
typename __testlib_enable_if<is_iterable<std::string>::value, void>::type __testlib_print_one<std::string>(
    const std::string &t) {
    std::cout << t;
}
...
void prepareOpts(int argc, char *argv[]) {
    if (argc <= 0) __testlib_fail("Opts: expected argc>=0 but found " + toString(argc));
    ...
    for (size_t index = 0; index < n; index++) __testlib_argv[index] = argv[index];
}
...
std::string __testlib_keyToOpts(const std::string &key) {
    if (__testlib_opts.count(key) == 0) __testlib_fail("Opts: unknown key '" + compress(key) + "'");
...
std::string parseExponentialOptValue(const std::string &s) {
    ...
    if (e.empty()) __testlib_fail("Opts: expected typical exponential notation but '" + compress(s) + "' found");
    if (e.length() > 20) __testlib_fail("Opts: expected typical exponential notation but '" + compress(s) + "' found");
    ...
    while (num.find('.') != std::string::npos && num.back() == '0') num = num.substr(0, num.length() - 1);
    if (!num.empty() && num.back() == '.') num = num.substr(0, num.length() - 1);
    if ((!num.empty() && num[0] == '.') || num.empty()) num.insert(num.begin(), '0');
    return (minus ? "-" : "") + num;
}
...
T optValueToIntegral(const std::string &s_, bool nonnegative) {
    ...
    if (s.empty()) __testlib_fail("Opts: expected integer but '" + compress(s_) + "' found");
    T value = 0;
    long double about = 0.0;
    signed char sign = +1;
    size_t pos = 0;
    if (s[pos] == '-') {
        if (nonnegative) __testlib_fail("Opts: expected non-negative integer but '" + compress(s_) + "' found");
        sign = -1;
        pos++;
    }
    for (size_t i = pos; i < s.length(); i++) {
        if (s[i] < '0' || s[i] > '9') __testlib_fail("Opts: expected integer but '" + compress(s_) + "' found");
...
long double optValueToLongDouble(const std::string &s_) {
    std::string s(parseExponentialOptValue(s_));
    if (s.empty()) __testlib_fail("Opts: expected float number but '" + compress(s_) + "' found");
    ...
    for (size_t i = pos; i < s.length(); i++) {
        ...
        if (period) mul *= 10.0;
        if (s[i] < '0' || s[i] > '9') __testlib_fail("Opts: expected float number but '" + compress(s_) + "' found");
        if (period)
            value += (s[i] - '0') / mul;
        else
            value = value * 10 + s[i] - '0';
...
bool opt(std::true_type, std::true_type, const std::string &key) {
    ...
    if (value == "true" || value == "1") return true;
    if (value == "false" || value == "0") return false;
    __testlib_fail("Opts: key '" + compress(key) + "': expected bool true/false or 0/1 but '" + compress(value) +
                   "' found");
}
